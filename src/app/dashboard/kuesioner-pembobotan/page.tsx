...
export default function KuesionerPembobotanPage() {
  ...
  const resetForm = () => {
    ...
    setExpertBg({
      nama: "", jenisKelamin: "", posisi: "", namaInstansi: "", jenisKeahlian: "",
      pengalaman: "", email: "", tanggal: today,
    });
    setComparisons({ "KU_LEVEL": {}, "CP_LEVEL": {} });
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            ...
            >
              <div className="p-6 text-center space-y-4 flex-shrink-0 border-b">
                ...
                <p className="text-sm text-muted-foreground">
                  Anda telah menyelesaikan seluruh kuesioner pembobotan. Berikut adalah rekap data yang Anda masukkan:
                </p>
              </div>
              ...
              <div className="p-6 overflow-y-auto space-y-6 bg-muted/20">
                ...
                <div className="space-y-3">
                  <h4 className="font-semibold text-sm border-b pb-2">Rekap Pembobotan (Jumlah Terisi)</h4>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    {Object.entries(comparisons).map(([mode, data]) => {
                      const answered = Object.values(data).filter(v => v !== undefined).length;
                      return (
                        <div key={mode} className="flex justify-between items-center col-span-2 bg-background p-3 rounded-xl border shadow-sm">
                          ...
                          <span className="text-cyan-500 font-bold">{answered} Perbandingan</span>
                        </div>
                      );
                    })}
                  </div>
